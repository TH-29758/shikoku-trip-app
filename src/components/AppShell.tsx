...
const navigation: { to: string; label: string; icon: IconName }[] = [
  { to: '/', label: 'ホーム', icon: 'home' }, { to: '/schedule', label: '旅のスケジュール', icon: 'calendar' },
  { to: '/map', label: 'スポット・地図', icon: 'map' }, { to: '/accommodations', label: '泊まるところ', icon: 'bed' },
  { to: '/party', label: 'みんなの会計', icon: 'wallet' }, { to: '/checklist', label: '持ち物チェック', icon: 'check' },
...
export function HeaderBar({ title }: { title: string }) {
  ...
  return <><header className="page-header"><div><span className="header-kicker">SHIKOKU / SEPTEMBER 2026</span><h1>{title}</h1></div><div className="header-right"><span className="header-date"><Icon name="calendar" size={16} />9.24 — 9.29</span><button className="icon-button mobile-menu" aria-label="メニューを開く" onClick={() => menu.current?.showModal()}><Icon name="menu" /></button><Link className="profile-chip" to="/etc" aria-label="名前とアプリ設定">{(readStorage('shikokuUserName') || '旅')[0]}</Link></div></header><dialog ref={menu} className="menu-dialog" aria-labelledby="menu-heading" onClick={event => { if (event.target === event.currentTarget) close(); }}><div className="menu-content"><div className="menu-title"><h2 id="menu-heading">旅のメニュー</h2><button autoFocus className="icon-button" onClick={close} aria-label="メニューを閉じる"><Icon name="close" /></button></div><Navigation onNavigate={close} /><p className="menu-caption">2026.09.24 — 09.29 / 6 DAYS</p></div></dialog></>;
}
export function AppShell({ children }: { children: ReactNode }) {
  const location = useLocation();
  useEffect(() => { if (!location.hash) window.scrollTo({ top: 0, behavior: 'instant' }); }, [location.pathname, location.hash]);
  return <div className="app-shell"><a className="skip-link" href="#main-content">本文へ移動</a><aside className="desktop-rail"><Brand /><div className="rail-trip-label">OUR TRAVEL NOTE <span>01</span></div><Navigation /><div className="rail-footer"><div className="rail-date"><span>SEPTEMBER</span><strong>24 <i>—</i> 29</strong><p>6日間、四国をぐるっと。</p></div><ConnectionState /></div></aside><main id="main-content" className="app-main" tabIndex={-1}>{children}</main><nav className="bottom-navigation" aria-label="主要ページ">{navigation.slice(0,5).map((item,index) => <NavLink to={item.to} key={item.to} end={item.to === '/'} className={({ isActive }) => isActive ? 'is-active' : ''}><Icon name={item.icon} size={21} /><span>{['ホーム','日程','地図','宿泊','会計'][index]}</span></NavLink>)}</nav></div>;
}
