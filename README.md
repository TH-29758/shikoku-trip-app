...
- src/components/: 共通ナビ、エラー画面、アプリ更新通知。
- src/pages/: ホーム、旅行情報、持ち物・会計、設定。
- src/lib/: Firebase、共有データ読み込み・保存、整数円の割り勘、端末内の名前保存。
- tests/: 日程とリンクの整合性、日本時間のカウントダウン、端数精算、過去記録、保存制限の検証。

Firestoreは既存の tripData/checklist、tripData/party と従来の配列形式を引き継ぎます。閲覧時にデータを書き込まず、変更操作のみトランザクションで保存。新しい支払いは正の整数円で入力し、過去の0円・返金記録も読み込みます。

共有データはGoogleログインした登録済み旅行メンバーに限定します。テストモードの期限切れ対応、Firebase側の設定・参加登録・ルール公開と検証手順は [FIREBASE_SETUP.md](FIREBASE_SETUP.md) を参照してください。Vercelへのアプリ公開とは別にFirestoreルールの公開が必要です。
...
WindowsでChromeが標準パスにインストールされている環境向けです。上のpreviewを起動した状態で node scripts/qa-browser.mjs を実行。専用の一時プロファイルを .qa/ に作り、全外部通信を遮断して主要画面・日程タブ・検索・旧リンク・設定・PWAオフライン表示を確認します。スクリーンショットと browser-report.json も .qa/ に保存されます。本番の共有データは更新しません。
